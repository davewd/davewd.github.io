...
const ThoughtsContainer: React.FC<ThoughtsContainerProps> = ({ activeSectionId }) => {
  const [searchParams, setSearchParams] = useSearchParams();
  const [selectedThought, setSelectedThought] = useState<Thought>(thoughtsData.thoughts[0]);
  const [selectedSection, setSelectedSection] = useState<Section>(selectedThought.sections[0]);

  useEffect(() => {
    // Find thought and section by ID from URL or default to first thought and section
    const thought = thoughtsData.thoughts.find(t => 
  ...
  const handleThoughtNavigation = (direction: 'prev' | 'next') => {
    const currentIndex = thoughtsData.thoughts.findIndex(t => t.id === selectedThought.id);
    const newIndex = direction === 'prev' 
      ? (currentIndex - 1 + thoughtsData.thoughts.length) % thoughtsData.thoughts.length
